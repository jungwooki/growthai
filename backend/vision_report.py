...
class StageReading(BaseModel):
    phase: Literal['pre_phv','accelerating','circa_phv','post_phv','uncertain'] = 'uncertain'
    status: Literal['estimated','insufficient']
    label: str
    system: str
    reasoning: str
...
4 phv: 근거자료의 실제 단계 정의와 관찰 또는 성장속도로 가능한 범위만 추정. 숫자 Stage를 붙일 경우 그 분류체계 정의가 참고자료에 존재해야 한다. Tanner Stage와 PHV Stage를 혼동하지 않는다. 정의가 없으면 숫자 없는 성장시기 설명 또는 판단 보류. 기존 Stage 2를 근거 없이 유지하지 않는다.
...
추가 운영 정책: 성장 단계가 주 평가이고 예상키는 보조 추정이다. phv.phase는 pre_phv, accelerating, circa_phv, post_phv, uncertain 중 근거로 분류하며 애매하면 uncertain이다. 계측·capping·실제 성장속도를 종합한다.
...
def validate_report(report,selected,visual,files,source_view):
    ...
    data['policy_version']='sports-growth-2026-09-23'
