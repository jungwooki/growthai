...
- Budget preference: KRW 50,000 monthly target across hosting, storage, and AI; notify and let the user decide additional spend. Do not upgrade plans or activate advanced AWS features automatically.

- help_level: MEDIUM (user selected; explain major decisions only).

- AWS CLI profile: `growthai`.
- Selected project Region: `ap-southeast-2` (user supplied).
- AWS experience: new AWS experience.
- `us-east-1` is used for the AWS Agent Toolkit/MCP service, not GrowAI regional resources.
- Never copy browser-login temporary credentials into deployment environment variables.
